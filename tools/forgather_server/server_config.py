...
# AI agent (right-sidebar assistant). Disabled until ``model`` is set.
# Uses the Anthropic SDK for both Claude and local vLLM models — point
# ``base_url`` at a vLLM server that serves the Anthropic Messages API
# (``/v1/messages``) for local models, or omit it for Claude.
# agent:
