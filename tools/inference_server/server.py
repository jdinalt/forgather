...
{%- endif -%}"""
    ...
    def generate_response(
        ...
        self.logger.info(
            f"[{request_id}] New chat completion request: model={request.model}, "
        ...
        self.logger.info(
            f"[{request_id}] Input tokens with special tokens: {repr(self.tokenizer.decode(input_token_ids, skip_special_tokens=False))}"
        )
        ...
        self.logger.info(
            f"[{request_id}] Generated tokens with special tokens: {repr(raw_generated_text)}"
        )
        ...
        if len(generated_token_ids) >= request.max_tokens:
            ...
            self.logger.info(
                f"[{request_id}] Generation stopped due to stop sequence: {repr(stop_sequence_found)}"
            )
        elif (
            ...
            self.logger.info(
                f"[{request_id}] Generation stopped due to stop token ID: {generated_token_ids[-1]}"
            )
        ...
        self.logger.info(
            f"[{request_id}] Token usage: prompt={prompt_tokens}, completion={completion_tokens}, total={prompt_tokens + completion_tokens}"
        )
    ...
    def generate_completion(self, request: CompletionRequest) -> CompletionResponse:
        ...
        self.logger.info(
            f"[{request_id}] New completion request: model={request.model}, "
            f"max_tokens={request.max_tokens}, temperature={request.temperature}, "
        ...
        all_stop_sequences = [
            s for s in (self.stop_sequences + request_stop_sequences) if s
        ]
        ...
        self.logger.info(f"[{request_id}] Generated token IDs: {generated_token_ids}")
        self.logger.info(
            f"[{request_id}] Generated tokens with special tokens: {repr(generated_text_with_special)}"
        )
    ...
    def generate_stream_response(self, request: ChatCompletionRequest) -> Iterator[str]:
        ...
        # Log request details
        self.logger.info(
            f"[{request_id}] New streaming chat completion request: model={request.model}, max_tokens={request.max_tokens}, temperature={request.temperature}, top_p={request.top_p}, messages_count={len(request.messages)}"
        )

        # Log each message
        for i, message in enumerate(request.messages):
            self.logger.info(
                f"[{request_id}] Message {i}: role={message.role}, content={repr(message.content)}"
            )
        ...
        try:
            ...
            self.logger.info(
                f"[{request_id}] Formatted prompt: {repr(formatted_prompt)}"
            )
            ...
            self.logger.info(
                f"[{request_id}] Input tokens with special tokens: {repr(self.tokenizer.decode(input_ids[0], skip_special_tokens=False))}"
            )

            # Build generation config
            generation_config = self._build_generation_config(request)
            self.logger.info(
                f"[{request_id}] Generation config: {repr(generation_config)}"
            )
                ...
                if new_text:  # Skip empty strings
                    ...
                    for stop_seq in self.stop_sequences:
                        if stop_seq in full_response:
                            ...
                            remaining_text = trimmed_response[
                                len(full_response) - len(new_text) :
                            ]
                            if remaining_text:
                                chunk = ChatCompletionStreamResponse(
                                    id=request_id,
            ...
            self.logger.info(
                f"[{request_id}] Generated token IDs: {generated_token_ids}"
            )
            self.logger.info(
                f"[{request_id}] Generated tokens with special tokens: {repr(full_response)}"
            )
            self.logger.info(
                f"[{request_id}] Response text (clean): {repr(full_response)}"
            )
    ...
    def generate_stream_completion(self, request: CompletionRequest) -> Iterator[str]:
        ...
        # Log request details
        self.logger.info(
            f"[{request_id}] New streaming completion request: model={request.model}, max_tokens={request.max_tokens}, prompt={repr(request.prompt)}"
        )
...
async def list_models():
    ...
    return {
        ...
        "data": [
            {
                "id": (
                    inference_server.model_path.split("/")[-1]
                    if inference_server
                    else "unknown"
                ),
                "object": "model",
...
def main():
    ...
    if isinstance(args.from_checkpoint, str):
        args.from_checkpoint = type = os.path.expanduser(args.from_checkpoint)
