...
- **[`examples/base_lm_project`](./examples/base_lm_project)** --
  ...
  new work.
- **[`examples/tiny_experiments/attention`](./examples/tiny_experiments/attention)**
  -- swap attention implementations (SDPA, flex-attention, custom
  variants) at tiny model scale for fast ablation.
- **[`examples/tiny_experiments/canon`](./examples/tiny_experiments/canon)**
  -- cross-project model inheritance with a `modelsrc/` override
  pattern.
- **[`examples/torchtitan`](./examples/torchtitan)** -- configure
  Torch Titan via Forgather (proof-of-concept interoperability).

## Command Overview

`forgather <subcommand>` dispatches everything; `forgather --help`
lists them all. The ones you'll use most:

| Command | Purpose |
|---------|---------|
| `forgather ls [-r]` | List configs in the current project (`-r` recursive) |
| `forgather -t X pp` | Show config X's fully-resolved YAML |
| `forgather -t X trefs` | Show the template inheritance tree for X |
| `forgather -t X train [-d GPUs]` | Train with config X |
| `forgather -t X tb` / `forgather tb --all` | Launch TensorBoard |
| `forgather control {list,status,save,stop,save-stop,abort}` | Manage running jobs |
| `forgather logs {summary,plot}` | Offline analysis of trainer_logs.json |
| `forgather -t X eval {list,show,test}` | Evaluate on a named eval config |
| `forgather inf {server,client}` | OpenAI-compatible server + CLI client |
| `forgather convert [--reverse --model-type llama] SRC DST` | Convert HF ↔ Forgather |
| `forgather checkpoint link` | Symlink latest checkpoint so plain `from_pretrained` works |
| `forgather project create` / `ws create` | Scaffold a new project or workspace |
| `forgather -i` | Interactive shell (no `forgather` prefix needed) |
...
- `src/forgather/` -- core framework
  ...
  - `ml/` -- training infrastructure (trainers, optimisers, callbacks, datasets)
- `templatelib/` -- reusable templates
  - `base/` -- abstract base templates (one level above the recommended ones)
  - `examples/` -- recommended recipes (`projects/lm_training_project.yaml`,
    `projects/finetune_v2.yaml`), tokenizers, callbacks, models
  - `finetune/` -- finetune-specific layers
- `modelsrc/` -- modular model components (transformer blocks,
  attention variants, rotary embeddings, etc.) used by the model
  templates

### Examples

Full directory map and usage notes: **[`examples/README.md`](./examples/README.md)**.
Curated by-journey list: [Featured Examples](#featured-examples) above.

- `examples/tutorials/` -- learning materials (start here)
- `examples/pretrain/` -- pretraining recipes
- `examples/finetune/` -- finetune recipes
- `examples/tiny_experiments/` -- fast ablation projects
- `examples/base_lm_project/` -- bare harness for the raw
  `projects/lm_training_project.yaml` template; useful for inspecting
  what the base template does on its own
- `examples/datasets/` -- dataset projects referenced by the recipes above
- `examples/models/` -- model-definition projects
- `examples/tokenizers/` -- tokenizer projects
- `examples/evaluation/` -- named eval configs used by
  `forgather eval test`
- `examples/snippets/` -- utility scripts (checkpoint management,
  run comparison, log concatenation)
- `examples/trainer_control/` -- minimal worked example of the live
  job-control API
- `examples/torchtitan/` -- Torch Titan interoperability

### Documentation

Full documentation index: **[`docs/README.md`](./docs/README.md)**.
Highest-priority pointers below.

**New to Forgather**

- [docs/getting-started/](./docs/getting-started/README.md) -- install
  and first training run (the authoritative install guide)
- [docs/core-concepts/](./docs/core-concepts/README.md) -- config
  pipeline, projects, templates, trainers

**Configuration language**

- [docs/configuration/syntax-reference.md](./docs/configuration/syntax-reference.md)
  -- line statements and YAML tags
- [docs/configuration/inheritance.md](./docs/configuration/inheritance.md)
  -- template inheritance patterns
- [docs/configuration/debugging.md](./docs/configuration/debugging.md)
...
- [docs/project-templates/lm-training-projects.md](./docs/project-templates/lm-training-projects.md)
  -- the `lm_training_project.yaml` parameter surface (most commonly referenced)

**Training**

- [docs/trainers/trainer_options.md](./docs/trainers/trainer_options.md)
  -- every training-argument and constructor parameter across trainers
- [docs/trainers/pipeline-parallel.md](./docs/trainers/pipeline-parallel.md)
  -- pipeline parallelism for consumer GPUs / limited-bandwidth multi-node
- [docs/trainers/trainer-control.md](./docs/trainers/trainer-control.md)
  -- save / stop / save-stop / abort for running jobs
- [docs/trainers/diloco.md](./docs/trainers/diloco.md) -- DiLoCo
  (distributed local SGD)
- [docs/trainers/fp8-training.md](./docs/trainers/fp8-training.md)
  -- FP8 training via torchao
- [docs/trainers/training-performance-metrics.md](./docs/trainers/training-performance-metrics.md)
  -- token throughput, FLOPs, MFU
- [docs/checkpointing/](./docs/checkpointing/README.md) -- distributed
  checkpointing abstraction + user / migration guides

**Models and data**

- [docs/model-architecture.md](./docs/model-architecture.md) -- inventory
  of transformer components in `modelsrc/transformer/`
- [docs/guides/model-conversion.md](./docs/guides/model-conversion.md)
  -- bidirectional HF ↔ Forgather conversion
- [docs/guides/update-vocab.md](./docs/guides/update-vocab.md) --
  adding tokens or chat templates to existing models
- [docs/datasets/sequence-packing.md](./docs/datasets/sequence-packing.md)
  -- packed-sequence mechanics

**How-to guides**

- [docs/guides/creating-a-model-project.md](./docs/guides/creating-a-model-project.md)
- [docs/guides/creating-a-dataset-project.md](./docs/guides/creating-a-dataset-project.md)
- [docs/guides/interactive-cli.md](./docs/guides/interactive-cli.md)
- [docs/guides/evaluating-models.md](./docs/guides/evaluating-models.md)
- [docs/logs-analysis.md](./docs/logs-analysis.md) -- programmatic log analysis
- [docs/inference/vllm_integration.md](./docs/inference/vllm_integration.md)
  -- (currently blocked on upstream Transformers v5 support)

## Development Setup

### Git hooks for code formatting

Forgather uses `isort` + `black` via a git pre-commit hook to keep
style consistent. **First-time setup (required for contributors):**

```bash
./scripts/setup-hooks.sh
```

After that, Python files are automatically formatted on every commit.
Add formatting exceptions to `.formatting-ignore` (Jinja2 templates
with `.py` extensions are already excluded).

Bypass formatting when you genuinely need to:

```bash
git commit --no-verify -m "..."
```

See [`.githooks/README.md`](./.githooks/README.md) for more.

### Testing

```bash
pytest                     # run the whole suite
pytest tests/unit/ml/      # narrow to one area
```

See [docs/development/testing.md](./docs/development/testing.md) for
the test-organisation guide.

## Contributing

Forgather is actively developed and welcomes contributions:

1. **Development setup** -- `./scripts/setup-hooks.sh` to enable git
   hooks; `pip install -e .` to install in editable mode.
2. **Bug reports & feature requests** -- open GitHub issues.
3. **Code contributions** -- submit pull requests.
4. **Documentation** -- improve tutorials, READMEs, and docs under
   `docs/`.
