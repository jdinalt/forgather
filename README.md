...
- **May 2026 (1.2.0)** -- **Multi-node forgather server**. `forgather server --cluster <name>` puts a node into cluster mode; peers discover each other over mDNS, peer-pull liveness elects a master, and a new `forgather cluster` CLI (`nodes` / `jobs` / `submit` / `cancel`) plus a Cluster panel in the web UI make it easy to coordinate multi-node jobs. One-click peer SSO from the sidebar, per-node restart/shutdown, raw-TCP bandwidth probe that hits wire-speed on 10 Gbps. See [`docs/guides/multi-node-training.md`](./docs/guides/multi-node-training.md).
- **May 2026 (1.2.0)** -- **Dataset server**. New long-running service that turns datasets into a cluster-shared resource: a unified view across nodes for multi-node training, registry-level load balancing, remote streaming for corpora that don't live on the local node, and the **stateful** dataset protocol -- O(1) continuation on resume instead of stream-and-discard to the saved position. Resilient client retries through transient errors and server restarts. Webui Tools entry + dedicated Datasets view; CLI as `forgather dataset-server`.
- **May 2026 (1.2.0)** -- **TLS / mTLS** across all three servers (forgather, dataset, inference). One `forgather tls init` per host, `forgather tls deploy` for clusters. Cluster peer-call authentication is now mutual-TLS, replacing the 1.1.0 source-IP carve-out. Docs: [`docs/operations/tls.md`](./docs/operations/tls.md).
- **May 2026 (1.2.0)** -- **torchao QAT + PTQ unified** as `forgather finalize --quantize <recipe>`. Same flag covers post-training quantization of any bf16 model and the QAT round-trip (train with `--qat-recipe`, deploy via `--quantize`). Native loader autodetects torchao quantization at load time -- no flag, no marker file -- so `eval test`, `inf server --from-checkpoint`, and trainer resume all work transparently. Full release notes: [`docs/release-notes/v1.2.0.md`](./docs/release-notes/v1.2.0.md).
