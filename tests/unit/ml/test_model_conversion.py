...
class TestForgatherMigrationChain(unittest.TestCase):
    ...
    @staticmethod
    def _make_converter(arch_version, migrations):
        from forgather.ml.model_conversion.base import ModelConverter
        ...
        _FakeConverter.arch_version = arch_version
    ...
    def test_missing_step_raises_with_clear_diagnostic(self):
        from forgather.ml.model_conversion import (
            VersionMigration,
            compose_migration_chain,
        )
    ...
    def test_migration_applies_config_and_state_dict_renames(self):
        ...
        def _v1_to_v2_config(cfg):
            new = dict(cfg)
            if "old_field" in new:
                new["mid_field"] = new.pop("old_field")
            return new

        def _v2_to_v3_config(cfg):
            new = dict(cfg)
            if "mid_field" in new:
                new["new_field"] = new.pop("mid_field")
            return new

        s1 = VersionMigration(
            description="rename old_field->mid_field; foo.* -> mid.*",
            migrate_config=_v1_to_v2_config,
            param_subs=((r"foo\.", r"mid.", ()),),
        )
        s2 = VersionMigration(
            description="rename mid_field->new_field; mid.* -> new.*",
        ...
        for _, step in chain:
            config = step.migrate_config(config)
            if step.param_subs:
                state_dict = remap_state_dict(state_dict, step.param_subs)

        self.assertNotIn("old_field", config)
        self.assertNotIn("mid_field", config)
        self.assertEqual(config["new_field"], 42)
        self.assertEqual(config["shared"], "keep_me")

        self.assertNotIn("foo.weight", state_dict)
        self.assertNotIn("mid.weight", state_dict)
        self.assertIn("new.weight", state_dict)
...
class TestModelConverterArchAttributes(unittest.TestCase):
    ...
    def test_default_arch_attributes(self):
        from forgather.ml.model_conversion.base import ModelConverter
    ...
    def test_subclass_can_override(self):
        from forgather.ml.model_conversion.base import (
            ModelConverter,
            VersionMigration,
        )
