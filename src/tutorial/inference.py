...
def show_colorized_tokens(
    ...
    metrics, metric_min, metric_max, metric_label = metric_fn(
        logits=logits, label_ids=label_ids
    )
    value_min, value_max = metrics.aminmax()

    print(
        f"Metric '{metric_label}': n={metrics.numel()}, min={value_min}, max={value_max}, mean={metrics.mean()}, range=({metric_min}, {metric_max})"
    )

    # if causal only; we need to pad the left with something to alighn the predictions.
    metrics = torch.cat(
        (
            torch.zeros(metrics.size(0), 1, device=metrics.device, dtype=metrics.dtype),
            metrics,
        ),
        dim=-1,
    )
    ...
    colors = color_encoder(
        metrics, value_min, value_max, metric_min=metric_min, metric_max=metric_max
    )
    ...
    for i in range(label_ids.size(0)):
        ...
        color_seq = colors[i]
        token_info_list = generate_token_info_list(
            tokenizer, token_seq, metric_label, metrics[i], logits[i], top_k
        )
...
class ColorEncoder:
    ...
    def normalize_metric(self, metric, minimum, maximum):
        return torch.clamp(
            input=(metric - minimum) / (maximum - minimum), min=0.0, max=1.0
        )
...
def generate_token_info_list(
    tokenizer, token_seq, metric_label, metrics, logits, top_k=5
):
