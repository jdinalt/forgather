...
class DataCollatorForCausalLM:
    ...
    def __init__(
        ...
        **pad_kwargs,
        ...
        if self.max_length > tokenizer.model_max_length:
            logger.warning(f"{max_length=} is greater than {tokenizer.model_max_length=}")
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        batch["labels"] = torch.where(input_ids == self.tokenizer.pad_token_id, self.ignore_index, input_ids)
        return batch

    def _truncate(self, features):
        if self.tokenizer.truncation_side == "right":
            features = [ { key: value[:self.max_length] for key, value in example.items() } for example in features ]
        else:
            features = [ { key: value[len(value) - self.max_length:] for key, value in example.items() } for example in features ]
        return features
