...
def make_shard_index(
    ...
    for shard_number, shard_weights in enumerate(shard_list):
        if safetensors:
            shard_file_name = f"model-{shard_number + 1:05}-of-{len(shard_list):05}.safetensors"
        else:
            shard_file_name = f"pytorch_model-{shard_number + 1:05}-of-{len(shard_list):05}.bin"
...
def validate_output_dir(
    ...
    if index_file_name:
        if not overwrite:
            raise Exception(f"Checkpoint exists '{output_dir}' exists and 'overwrite' is False")
        else:
            logger.warning(
...
def load_checkpoint(
    ...
    if is_safetensors:
        ...
        state_dict = torch.load(state_dict_path, map_location=device, weights_only=True, mmap=True)
...
def load_sharded_checkpoint(
    ...
    if strict and len(missing_keys):
        raise Exception(f"Index file does not contain mappings for the following keys {missing_keys} ")
    ...
    for shard_file_name in shard_files:
        ...
        if safetensors:
            ...
            state_dict = torch.load(shard_file_path, map_location=device, weights_only=True, mmap=True)
...
def checkpoint_exists(model_dir: str) -> Tuple[str|NoneType, bool|NoneType, bool|NoneType]:
    """
