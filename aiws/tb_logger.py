...
class TBLogger(TrainerCallback):
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        self.summary_writer.add_text("training_info", self.mapping_as_markdown(info | extra_info))
    ...
    def on_evaluate(self, args, state, control, metrics, **kwargs):
        ...
        self.summary_writer.add_scalar('eval-loss', metrics['eval_loss'], global_step=global_step)
        if metrics.get('eval_accuracy') is not None:
            self.summary_writer.add_scalar('eval-accuracy', metrics['eval_accuracy'], global_step=global_step)
        try:
    ...
    def on_log(self, args, state, control, logs, **kwargs):
        ...
        global_step = state.global_step
        if 'loss' in logs:
            self.summary_writer.add_scalar('train-loss', logs['loss'], global_step=global_step)
        if 'learning_rate' in logs:
            self.summary_writer.add_scalar('learning-rate', logs['learning_rate'], global_step=global_step)
        self.summary_writer.flush()

    def on_train_end(self, args, state, control, **kwargs):
        if not state.is_world_process_zero and len(state.log_history):
            return
        self.summary_writer.add_text("train_results", self.mapping_as_markdown(state.log_history[-1]))