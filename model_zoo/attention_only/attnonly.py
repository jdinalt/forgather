...
from aiws import attninspector
from aiws.dotdict import DotDict

class AttnOnlyAttnInspector(attninspector.AttnInspector):
    def __init__(self, parent, attn):
        self.parent = parent
        self.attn = attn

    def num_heads(self):
        return self.attn.num_heads

    def head_dims(self):
        return DotDict({ "query": self.attn.d_head, "key": self.attn.d_head })

    def dot_product_scale(self):
        return self.attn.dot_product_scale

    def weights(self, head=None):
        q = self.attn.query_linear.weight
        k = self.attn.key_linear.weight
        v = self.attn.value_linear.weight
        return DotDict({ "query": q, "key": k, "value": v, "output": None })

    def biases(self, head=None):
        q = self.attn.query_linear.bias
        k = self.attn.key_linear.bias
        v = self.attn.value_linear.bias
        return DotDict({ "query": q, "key": k, "value": v, "output": None })

class AttnOnlyLayerInspector(attninspector.LayerInspector):
    def __init__(self, parent, layer):
        self.parent = parent
        self.layer = layer

    def attention(self):
        return AttnOnlyAttnInspector(self, self.layer.attention)

class AttnOnlyModelInspector(attninspector.ModelInspector):
    ...
    def positional_encoder(self):
        return self.model.positional_encoder

    def layer(self, l):
        return AttnOnlyLayerInspector(self, self.model.layers[l])
...
model_type = "attn-only-transformer"
...
class TransformerModel(PreTrainedModel):
    ...
    def inspector(self):
        return AttnOnlyModelInspector(self)
...
AutoConfig.register(model_type, TransformerConfig)
AutoModelForCausalLM.register(TransformerConfig, TransformerModel)
