...
def eager_scaled_dot_product_attention(
    ...
    if seq_len != seq_len_k or seq_len != seq_len_v:
        raise ValueError(f"Sequence length mismatch: query={seq_len}, key={seq_len_k}, value={seq_len_v}")
    if head_dim != head_dim_k or head_dim != head_dim_v:
        raise ValueError(f"Head dimension mismatch: query={head_dim}, key={head_dim_k}, value={head_dim_v}")
    if num_heads_kv != num_heads_kv_v:
        raise ValueError(f"Key and value must have same number of heads: key={num_heads_kv}, value={num_heads_kv_v}")
    
    # Validate GQA constraints
    if enable_gqa:
        if num_heads_q % num_heads_kv != 0:
            raise ValueError(f"For GQA, number of query heads ({num_heads_q}) must be divisible by number of key/value heads ({num_heads_kv})")
    
    # Handle GQA by expanding key and value tensors to match query heads
    if num_heads_q != num_heads_kv:
        if not enable_gqa:
            raise ValueError(f"Number of heads mismatch: query={num_heads_q}, key/value={num_heads_kv}. Use enable_gqa=True for Grouped Query Attention.")
